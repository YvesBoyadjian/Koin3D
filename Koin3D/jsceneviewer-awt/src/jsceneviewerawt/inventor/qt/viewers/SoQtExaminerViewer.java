...
import jsceneviewerawt.QDoubleSpinBox;
import jsceneviewerawt.inventor.qt.SoQtCameraController.Type;
import jsceneviewerawt.inventor.qt.SoQtIcons;
import jsceneviewerawt.inventor.qt.SoQtThumbWheel;
import jsceneviewerawt.inventor.qt.devices.SoQtMouseWheel;
    ...
    private void commonConstructor()
{
    ...
    setCursorShape (ViewerModes.PICK_MODE.ordinal(),  new Cursor(Cursor.DEFAULT_CURSOR));
...
{
    resetInteractions();
    setInteraction ((KeyEvent.BUTTON1_DOWN_MASK | KeyEvent.BUTTON2_DOWN_MASK), 0, ViewerModes.DOLLY_MODE_ACTIVE);
    setInteraction ((KeyEvent.BUTTON1_DOWN_MASK) | (KeyEvent.SHIFT_DOWN_MASK), 0, ViewerModes.PAN_MODE_ACTIVE);
    setInteraction ((KeyEvent.BUTTON1_DOWN_MASK) | (KeyEvent.CTRL_DOWN_MASK), 0, ViewerModes.PAN_MODE_ACTIVE);
    setInteraction ((KeyEvent.BUTTON1_DOWN_MASK) | (KeyEvent.SHIFT_DOWN_MASK | KeyEvent.CTRL_DOWN_MASK), 0, ViewerModes.DOLLY_MODE_ACTIVE);
    setInteraction ((KeyEvent.BUTTON1_DOWN_MASK), 0, ViewerModes.SPIN_MODE_ACTIVE);
    setInteraction ((KeyEvent.BUTTON2_DOWN_MASK) | (KeyEvent.CTRL_DOWN_MASK), 0, ViewerModes.DOLLY_MODE_ACTIVE);
    setInteraction ((KeyEvent.BUTTON2_DOWN_MASK), 0, ViewerModes.PAN_MODE_ACTIVE);
    setInteraction ((KeyEvent.SHIFT_DOWN_MASK | KeyEvent.CTRL_DOWN_MASK), 0, ViewerModes.DOLLY_MODE);
    setInteraction ((KeyEvent.SHIFT_DOWN_MASK), 0, ViewerModes.PAN_MODE);
    setInteraction ((KeyEvent.CTRL_DOWN_MASK), 0, ViewerModes.PAN_MODE);
...
{
    return ((inputState & KeyEvent.BUTTON1_DOWN_MASK)!= 0      ? 1  : 0) |
           ((inputState & KeyEvent.BUTTON3_DOWN_MASK)!= 0     ? 2  : 0) |
           ((inputState & KeyEvent.BUTTON2_DOWN_MASK)!= 0       ? 4  : 0) |
           ((inputState & KeyEvent.SHIFT_DOWN_MASK)!= 0   ? 8  : 0) |
           ((inputState & KeyEvent.CTRL_DOWN_MASK)!= 0 ? 16 : 0) |
           ((inputState & KeyEvent.ALT_DOWN_MASK)!= 0     ? 32 : 0);
}
...
protected void createViewerButtons(JToolBar parent)
{
...
public void setCamera(SoCamera newCamera, boolean createdCamera)
{
        ...
        newCamera.getTypeId() != camera.getTypeId()))
    {
        if (newCamera.isOfType(SoOrthographicCamera.getClassTypeId())) {
            perspAction.putValue(Action.SMALL_ICON, SoQtIcons.getIcon (SoQtIcons.IconNum.ICON_ORTHO.ordinal()));
            setRightWheelTitle("Zoom");
        } else {
            perspAction.putValue(Action.SMALL_ICON, SoQtIcons.getIcon (SoQtIcons.IconNum.ICON_PERSP.ordinal()));
...
protected void processEvent (ComponentEvent anyEvent, EventType type, final boolean[] isAccepted)
{
    ...
    if (type == EventType.MOUSE_EVENT_MOUSE_DOWN)//QEvent.MouseButtonPress)
    {
        ...
        locator.copyFrom( new SbVec2s((short)(me.getX() * raViewScale), (short)(raSize.getValue()[1] - (me.getY() * raViewScale))));

        if ((me.getButton() == 1 || me.getButton() == 2) &&
        ...
        updateViewerMode (me.getModifiersEx(),
                (me.getModifiersEx() & (MouseEvent.BUTTON1_DOWN_MASK | MouseEvent.BUTTON1_DOWN_MASK | MouseEvent.BUTTON1_DOWN_MASK) ));
    } else if (type == EventType.KEY_EVENT_KEY_PRESSED/*QEvent.KeyPress*/) {
        KeyEvent ke = (KeyEvent )anyEvent;
        updateViewerMode (ke.getModifiersEx(), ke.getModifiersEx() & (MouseEvent.BUTTON1_DOWN_MASK | MouseEvent.BUTTON1_DOWN_MASK | MouseEvent.BUTTON1_DOWN_MASK));
    } else if (type == EventType.MOUSE_EVENT_MOUSE_MOVE/*QEvent.MouseMove*/) {
        MouseEvent me = (MouseEvent) anyEvent;
        final SbVec2s pos = new SbVec2s((short)(me.getX() * raViewScale), (short)(raSize.getValue()[1] - (me.getY() * raViewScale)));
        if ((me.getModifiersEx() & (MouseEvent.BUTTON1_DOWN_MASK | MouseEvent.BUTTON1_DOWN_MASK | MouseEvent.BUTTON1_DOWN_MASK)) ==0) {
    ...
    if (type == EventType.KEY_EVENT_KEY_RELEASED/*QEvent.KeyRelease*/) {
        KeyEvent ke = (KeyEvent )anyEvent;
        updateViewerMode (ke.getModifiersEx(), ke.getModifiersEx() & (InputEvent.BUTTON1_DOWN_MASK | InputEvent.BUTTON1_DOWN_MASK | InputEvent.BUTTON1_DOWN_MASK));
    } else if (type == EventType.MOUSE_EVENT_MOUSE_UP/*QEvent.MouseButtonRelease*/) {
        MouseEvent me = (MouseEvent) anyEvent;
        if ((me.getButton() == 1/*Qt.LeftButton*/ || me.getButton() == 2/*Qt.MidButton*/) &&
            mode == ViewerModes.SEEK_MODE.ordinal())
        {
            //me.accept();
        } else { //... ButtonRelease
        ...
        updateViewerMode (me.getModifiersEx(), me.getModifiersEx() & (InputEvent.BUTTON1_DOWN_MASK | InputEvent.BUTTON2_DOWN_MASK | InputEvent.BUTTON3_DOWN_MASK ));
    }
}
...
private void updateViewerMode (int modifiers, int buttons)
{
    ...
    if (buttons == 0 && getCameraController().isSeekMode()) {
        ...
        if (isAltSwitchingEnabled()) {
            ...
            // it wasn't pressed:
            modifiers = (modifiers | KeyEvent.ALT_DOWN_MASK) ^ KeyEvent.ALT_DOWN_MASK;
        }
        ...
        case PICK_MODE:
            if (mouseInside) {
                ...
                if (prevMode != ViewerModes.SEEK_MODE.ordinal()) {
                    ...
                    // by a mouse press!
                    if ((buttons & KeyEvent.BUTTON1_DOWN_MASK)!=0/*Qt::LeftButton*/) { interactiveCountDec(); }
                    if ((buttons & KeyEvent.BUTTON2_DOWN_MASK)!=0/*Qt::MidButton*/)  { interactiveCountDec(); }
                }
                if ((buttons & KeyEvent.BUTTON3_DOWN_MASK)!=0/*Qt::RightButton*/) { interactiveCountDec(); }
            }
            stopAnimating();
            break;
