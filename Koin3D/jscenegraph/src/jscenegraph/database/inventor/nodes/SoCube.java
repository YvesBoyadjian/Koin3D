...
////////////////////////////////////////////////////////////////////////
{
  ...
  for (face = 0; face < 6; face++) {
    ...
    for (vert = 0; vert < 4; vert++)
      gl2.glVertex3fv((SCALE(verts[face][vert],tmp,scale).operator_add(translate)).getValue(),0);
...
private SbVec3f SCALE( SbVec3f vec, SbVec3f tmp, SbVec3f scale) {
	float[] pt = vec.getValue();
	tmp.getValue()[0] = (pt)[0]*scale.getValue()[0];
			tmp.getValue()[1] = (pt)[1]*scale.getValue()[1]; 
		   tmp.getValue()[2] = (pt)[2]*scale.getValue()[2];
...
////////////////////////////////////////////////////////////////////////
{
  ...
  // scaling by 0
  if (size.getValue()[0] == 0.0)
    size.getValue()[0] = 0.00001f;
  if (size.getValue()[1] == 0.0)
    size.getValue()[1] = 0.00001f;
  if (size.getValue()[2] == 0.0)
                                 ...
                                 boolean sendNormals, boolean doTextures)
{
  ...
  for (int face = 0; face < 6; face++) {
    ...
    for (int tri = 0; tri < 6; tri++) {
      ...
      verticesPtr.asterisk( (verts[face][vert]).getValue()[0]*scale.getValue()[0]); verticesPtr.plusPlus();
      verticesPtr.asterisk( (verts[face][vert]).getValue()[1]*scale.getValue()[1]); verticesPtr.plusPlus();
      verticesPtr.asterisk( (verts[face][vert]).getValue()[2]*scale.getValue()[2]); verticesPtr.plusPlus();
    }
  }
...
////////////////////////////////////////////////////////////////////////
{
	...
	if (numDivisions == 1) {
	    ...
	    vert = 3;
	    pt.setValue((verts[face][vert]).getValue()[0] * w[0],
			(verts[face][vert]).getValue()[1] * h[0],
			(verts[face][vert]).getValue()[2] * d[0]);
	    if (genTexCoords) {
...
////////////////////////////////////////////////////////////////////////
{
	...
	if (numDivisions == 1) {
		...
		gl2.glVertex3fv(SCALE(verts[face][vert],tmp,scale).getValue(),0);
	    }
	}
	...
	// More than one polygon per face
		...
		if (doTextures) {
		    ...
		    gl2.glVertex3fv(SCALE(topPoint,tmp,scale).getValue(),0);
		    gl2.glTexCoord2f(s, strip * di);
		    gl2.glVertex3fv(SCALE(botPoint,tmp,scale).getValue(),0);
		}
		else {
		    gl2.glVertex3fv(SCALE(topPoint,tmp,scale).getValue(),0);
		    gl2.glVertex3fv(SCALE(botPoint,tmp,scale).getValue(),0);
