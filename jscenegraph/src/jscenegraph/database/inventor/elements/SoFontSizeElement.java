 ...
 *
 */
public class SoFontSizeElement extends SoFloatElement {
	
    //! set the current font size
	public static void         set(SoState state, float size)
        { SoFloatElement.set(classStackIndexMap.get(SoFontSizeElement.class), state, size); }

    //! return the current font name from the state
    public static float        get(SoState state)
        { return SoFloatElement.get(classStackIndexMap.get(SoFontSizeElement.class), state); }
	

    //! Returns the default font size
    public static float        getDefault()    { return 10.0f; }
